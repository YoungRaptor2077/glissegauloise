        ...
        try {
          ...
          if (resend) {
            ...
            const lineItemsHtml = lineItems.map((item: any) =>
              `<tr>
                <td style="padding: 8px 0; border-bottom: 1px solid #2a2a2a; color: #f5f5f0;">${item.description || "Article"}${item.note ? `<br/><span style="font-size: 11px; color: #aaa; font-style: italic;">${item.note}</span>` : ""}${item.link ? `<br/><a href="${item.link}" target="_blank" rel="noopener noreferrer" style="display: inline-block; margin-top: 4px; padding: 2px 8px; background: rgba(0,255,136,0.1); color: #00ff88; font-size: 11px; border-radius: 4px; text-decoration: none;">Voir la piece</a>` : ""}</td>
